...
export async function cacheRequest(
  ...
  const isUnderProtectedLayer =
    url.pathname.replace(`/${env.PROTECTED.PROXY_KEYWORD}/`, '').split('/').length <=
    env.PROTECTED.PROTECTED_LAYERS;
  const isTokenValid = await authenticateToken(env.PASSWORD, url, []);
  const tokenScopeList = url.searchParams.get('scopes')?.split(',') || [];

  // validations
  const isProtectedWithoutToken = isUnderProtectedLayer && !isTokenValid;
  const isTokenProvidedButInvalid = url.searchParams.has('token') && !isTokenValid;
  const isAllowedGetRequest =
    method === 'GET' && (tokenScopeList.length === 0 || tokenScopeList.includes('download'));

  if (isProtectedWithoutToken || isTokenProvidedButInvalid || !isAllowedGetRequest) {
  ...
  cacheUrl.pathname = `/${method}` + url.pathname + key;
  ...
  const shouldForceRefresh = tokenScopeList.includes('refresh') && isTokenValid;

  if (!cachedResponse || isCacheExpired || shouldForceRefresh) {
