...
const localEnv = {}; // Generated by deployment helper
// jsonc no need JSON.parse, but toml need
const PROTECTED = globalEnv.PROTECTED || localEnv.PROTECTED;
const OAUTH = globalEnv.OAUTH || localEnv.OAUTH;
...
async function cacheRequest(request, env, ctx) {
  ...
  const CACHE_TTLMAP = env.CACHE_TTLMAP || localEnv.CACHE_TTLMAP;
...
async function fetchAccessToken() {
  let refreshToken = globalEnv.REFRESHTOEKN || localEnv.OAUTH?.refreshToken;
...
async function authenticate(path, passwd, davAuthHeader, davCredentials) {
  ...
  const pwFileContent = await downloadFile(`${path}/${PROTECTED.PASSWD_FILENAME}`, true, null).then(
...
async function downloadFile(filePath, stream, format, rangeHeader) {
  ...
  const res = await fetchWithAuth(uri, { headers });
  const downloadResHeaders = new Headers();
  downloadResHeaders.set('Last-Modified', new Date().toUTCString());
  ...
  if (!stream) {
    downloadResHeaders.set('Location', res.url);
    return new Response(null, {
      status: 302,
      headers: downloadResHeaders,
    });
  }

  const fileRes = stream ? await fetch(res.url, { headers }) : null;
  const resBody = stream ? fileRes?.body : null;
  if (fileRes) {
    const forwardHeaders = ['Content-Type', 'Content-Length', 'Accept-Ranges'];
    forwardHeaders.forEach((header) => {
      const value = fileRes.headers.get(header);
      if (value) downloadResHeaders.set(header, value);
    });
  }

  return new Response(resBody, {
    status: fileRes ? fileRes.status : 500,
    headers: downloadResHeaders,
  });
}
...
async function handleWebdav(filePath, request, davCredentials) {
  const davAuth = await authenticate(
    null,
    null,
...
async function handleCopyMove(filePath, method, destination) {
  ...
  const responseXML =
    davStatus === 201 ? null : createReturnXml(uriPath, davStatus, res.statusText);
